...
export default function SensitiveDataScanner() {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/scan-file', formData, {
  ...
  const fetchScanResults = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/scan-results');
  ...
  const handleDeleteResult = async (id: string) => {
    try {
      await axios.delete(`http://localhost:8000/api/scan-results/${id}`);
